...
#define BEGIN_ABSTRACT_FACTORY() \
  class UAbstractFactory : public IAbstractFactory {

#define END_FACTORY() };

#define CTOR_ABSTRACT_FACTORY(action) public: UAbstractFactory() { action }
...
#define SUBS_FACTORY(Type, ...) \
    public: virtual mem::TScope<IFactory<Type, __VA_ARGS__>> Get##Type() = 0;

#define SUBS_FACTORY_NAMESPACE(Namespace, Type, ...) \
    public: virtual mem::TScope<IFactory<Namespace::Type, __VA_ARGS__>> Get##Type() = 0;
